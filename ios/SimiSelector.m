...
RCT_EXPORT_METHOD(openSelector:(NSDictionary *)options
  ...
  @try {
    ...
    if (options != nil && [options isKindOfClass:[NSDictionary class]]) {
      ...
      if (options[@"selectMimeType"]) {
        selectMimeType = [options[@"selectMimeType"] intValue]; //0: all , 1: image , 2: video , 3: audio
      }
...
- (void)openPhotoPickerAllowMixSelect:(BOOL)allowMixSelect
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    uiConfig.themeColor = [UIColor colorWithRed:16.0/255.0 green:175.0/255.0 blue:255.0/255.0 alpha:1];
    uiConfig.customImageNames = @[@"zl_btn_selected", @"zl_btn_unselected", @"zl_btn_original_circle", @"zl_btn_original_selected"];
    uiConfig.customImageForKey_objc = @{@"zl_btn_selected" : [UIImage imageNamed:@"zl_btn_selected"],
                                        @"zl_btn_unselected" : [UIImage imageNamed:@"zl_btn_unselected"],
                                        @"zl_btn_original_circle" : [UIImage imageNamed:@"zl_btn_original_circle"],
                                        @"zl_btn_original_selected" : [UIImage imageNamed:@"zl_btn_original_selected"]};
    
    
    ZLPhotoConfiguration *config = [ZLPhotoConfiguration default];
    config.allowSelectImage = YES;
    config.allowSelectVideo = canSelectVideo ? (self.images.count == 0 ? YES : NO) : NO;
...
- (void)handlePickedResults:(NSArray<ZLResultModel *> *)results completion:(void (^)(id result))completion {
  ...
  dispatch_group_notify(group, dispatch_get_main_queue(), ^{
    ...
    weakSelf.hasSelectVideo = selectedAssets.firstObject.mediaType == PHAssetMediaTypeVideo;
    [weakSelf.selectedMedias addObjectsFromArray:[NSMutableArray arrayWithArray:tempMedias]];
...
- (void)processAsset:(ZLResultModel *)result
  ...
  [ZLPhotoManager fetchAssetFilePathFor:asset completion:^(NSString * _Nullable path) {
    if (path) {
      media[@"uri"] = path;
    }
    ...
    // 名称
    NSArray<PHAssetResource *> *resources = [PHAssetResource assetResourcesForAsset:result.asset];
    ...
    if (asset.mediaType == PHAssetMediaTypeVideo) {
      dispatch_group_enter(group);
      [self generateVideoThumbnailForAsset:asset group:group completion:^(NSString * _Nullable thumbnailPath) {
        if (thumbnailPath) {
...
- (void)generateVideoThumbnailForAsset:(PHAsset *)asset
                                 group:(dispatch_group_t)group
                            completion:(void (^)(NSString * _Nullable thumbnailPath))completion {
  ...
  [ZLPhotoManager fetchImageFor:asset size:thumbnailSize progress:nil completion:^(UIImage * _Nullable image, BOOL isDegraded) {
    if (image && !isDegraded) {
      NSString *path = [SimiSelector saveThumbnailToTemporaryDirectory:image];
      completion(path);
    } else {
      completion(nil);
    }
...
RCT_EXPORT_METHOD(clearPhotos:(BOOL)clearPhotos
                  clearVideos:(BOOL)clearVideos
                  resolve:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  // 清空数组
